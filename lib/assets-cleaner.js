...
const {info, logWithSpinner, stopSpinner, warn} = require('@vue/cli-shared-utils');

module.exports = () => {
  logWithSpinner(`Cleaning Assets...`);
  ...
  mediaDirs.forEach(mediaDir => {
    ...
    mediaDirContentDirs.forEach(mediaDirContentDir => {
      ...
      const isMediaDirForCurrentSlide = mediaDirContentDir.name === process.env.VUE_APP_OUT_DIR_PATH;
  ...
  logWithSpinner(`Assets clean`);
  stopSpinner();
};

function getDirectories(inPath) {
  if (!fs.existsSync(inPath)) {
    warn(`Dir "${inPath}"  not ready.`);
