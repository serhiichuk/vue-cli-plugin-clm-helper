...
module.exports = class ExelParser {
  ...
  run() {
    ...
    convertExcel(exelPath, null, parserOptions, (err, data) => {
      ...
      const newConfig = `module.exports = ${JSON.stringify({ ...require(paths.clm.config), ...clmConfig }, null, '  ')};`;

      fs.writeFileSync(paths.clm.config, newConfig);
      info(`"clm.config" was generated from "${chalk.yellow(path.relative(paths.root, exelPath))}".`);
    });
  }
